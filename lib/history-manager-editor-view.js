...
export default class HistoryManagerEditorView {
	...
	CreateModal()
	{
		...
		const objHeader = document.createElement('header');
		objHeader.classList.add(`${this.strClass}--header`);
		this.element.appendChild(objHeader);

		const objHead = document.createElement('h1');
		objHead.textContent = 'History Manager Editor';
		objHead.classList.add(`${this.strClass}--heading`);
		objHeader.appendChild(objHead);
		...
		const objHeadBtns = document.createElement('span');
		objHeadBtns.classList.add('btn-toolbar');
		objHeader.appendChild(objHeadBtns);

		const objReload = document.createElement('button');
		objReload.setAttribute('type', 'button');
		objReload.classList.add('btn', 'icon', 'icon-repo-sync');
		objReload.addEventListener('click', this.GetHistory.bind(this));
		objReload.addEventListener('mouseover', e => this.setTooltip(e.target, true, 'Reload History', 'reload'));
		objHeadBtns.appendChild(objReload);

		const objClose = document.createElement('button');
		objClose.setAttribute('type', 'button');
		objClose.classList.add('btn', 'icon', 'icon-x');
		objClose.addEventListener('click', () => {

			this.modalPanel.hide();

		});
		objHeadBtns.appendChild(objClose);
	...
	setTooltip(objEl, blnShow, strTitle, strIndex)
	{
		...
		if(this.tooltips[strIndex] === undefined || atom.tooltips.findTooltips(objEl)[0] === undefined)
		{
